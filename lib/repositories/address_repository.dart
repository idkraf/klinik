...
import 'package:klinikkecantikan/data_model/address_response.dart';
import 'package:klinikkecantikan/data_model/address_add_response.dart';
import 'package:klinikkecantikan/data_model/address_update_response.dart';
import 'package:klinikkecantikan/data_model/address_update_location_response.dart';
import 'package:klinikkecantikan/data_model/address_delete_response.dart';
import 'package:klinikkecantikan/data_model/address_make_default_response.dart';
import 'package:klinikkecantikan/data_model/address_update_in_cart_response.dart';
import 'package:klinikkecantikan/data_model/shipping_cost_response.dart';
import 'package:klinikkecantikan/helpers/shared_value_helper.dart';
import 'package:flutter/foundation.dart';
//rajaongkir modeling new
import 'package:klinikkecantikan/data_model/kabupaten_response.dart';
import 'package:klinikkecantikan/data_model/province_response.dart';
import 'package:klinikkecantikan/data_model/kecamatan_model.dart';
...
class AddressRepository {
  ...
  Future<CityResponse> getCityListByState({state_id = 0, name = ""}) async {
    Uri url = Uri.parse("${AppConfig.BASE_URL}/cities-by-state/${state_id}?name=${name}");
  ...
  Future<MyStateResponse> getStateListByCountry(
    ...
    Uri url = Uri.parse("${AppConfig.BASE_URL}/states-by-country/${country_id}?name=${name}");
    final response = await http.get(url);
  ...
  Future<ShippingCostResponse> getShippingCostResponse(
      {destinasi=0,
        shipping_type =""}) async {
    print("cek dest:$destinasi and cart:${shipping_type}");
  ...
  //Province Raja Ongkir
  Future<ProvinceResponse> getProvinsi() async {
    Uri url = Uri.parse("${AppConfig.BASE_URL}/provinsi");
    final response = await http.get(url);
    print(url.toString());
    print(response.body.toString());
  ...
  //kabupaten Raja Ongkir
  Future<KabupatenResponse> getKabupaten({prov_id = 0}) async {
    Uri url = Uri.parse("${AppConfig.BASE_URL}/kabupaten?id=${prov_id}");
    final response = await http.get(url);
    print(url.toString());
    print(response.body.toString());
    return kabupatenResponseFromJson(response.body);
  }

  //kecamatan Raja Ongkir
  Future<KecamatanResponse> getKecamatan({kab_id = 0}) async {
    Uri url = Uri.parse("${AppConfig.BASE_URL}/kecamatan?id=${kab_id}");
    final response = await http.get(url);
    print(url.toString());
    print(response.body.toString());
    return kecamatanResponseFromJson(response.body);
  }
