...
import 'package:klinikkecantikan/screens/cart.dart';
import 'package:klinikkecantikan/screens/common_webview_screen.dart';
import 'package:klinikkecantikan/screens/login.dart';
import 'package:klinikkecantikan/screens/product_reviews.dart';
import 'package:klinikkecantikan/ui_elements/list_product_card.dart';
...
import 'package:klinikkecantikan/helpers/color_helper.dart';
...
import 'package:klinikkecantikan/repositories/chat_repository.dart';
import 'package:klinikkecantikan/screens/chat.dart';
import 'package:toast/toast.dart';
import 'package:social_share/social_share.dart';
import 'dart:async';
import 'package:klinikkecantikan/screens/video_description_screen.dart';
import 'package:photo_view/photo_view.dart';
import 'package:klinikkecantikan/screens/brand_products.dart';
...
class _ProductDetailsState extends State<ProductDetails> with TickerProviderStateMixin{
  ...
  setProductDetailValues() {
    if (_productDetails != null) {
      ...
      _singlePriceString = _productDetails.main_price.toString().replaceAll("Rp", "").replaceAll(".", "").replaceAll(",", "");

      _stock = _productDetails.current_stock;
      _stock_preorder = _productDetails.current_stock_preorder;
      ...
      _wholesaleList = [];
      _productDetails.wholesale.forEach((grosir){
        _wholesaleList.add(grosir);
      });
  ...
  calculateTotalPrice() {
    if (_productDetails.wholesale.length > 0) {

      _totalPrice = (_singlePrice * _quantity).toStringAsFixed(2);
      _singlePriceString = _productDetails.main_price.toString().replaceAll("Rp", "").replaceAll(".", "").replaceAll(",", "");
      _wholesaleList.forEach((element) {
        dynamic minQty = element['min_qty'];
        dynamic maxQty = element['max_qty'];
        dynamic wprice = double.parse(element['price']);

        if(_quantity >= int.parse(minQty) && _quantity <= int.parse(maxQty)){
          print("cek min: $minQty");
          //_singlePrice = wprice;
          _totalPrice = (wprice * _quantity).toStringAsFixed(2);
          _singlePriceString = wprice.toString();
        }
        //else{
        //  print("cek min: $minQty");
          //_singlePrice = _productDetails.calculable_price;
  ...
  fetchAndSetVariantWiseInfo({bool change_appbar_string = true}) async {
    ...
    _stock_preorder = variantResponse.stock_preorder!;
    var jumlah = _stock + _stock_preorder;
    if (_quantity > jumlah) {
      _quantity = _stock;
      setState(() {});
    }
    ...
    calculateTotalPrice();
    _singlePriceString = variantResponse.price_string.toString().replaceAll("Rp", "").replaceAll(".", "");
                    ...
                    children: [
                      Padding(
                        ...
                        child: TextButton(
                          ...
                          onPressed: () {
                            onCopyTap(setState);
                            SocialShare.copyToClipboard(text:_productDetails.link);
                          },
                        ),
                      ),
                      _showCopied
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          padding: const EdgeInsets.only(bottom: 8.0),
                          child: Text(
                              AppLocalizations.of(context)!.product_details_screen_seller_chat_title,
                              style: TextStyle(
  ...
  onPressSendMessage() async {
    ...
    if (title == "" || message == "") {
      ToastComponent.showDialog(
          AppLocalizations.of(context)!.product_details_screen_seller_chat_title_message_empty_warning,
          gravity: Toast.center,
          duration: Toast.lengthLong);
      return;
    }
    ...
    if (conversationCreateResponse.result == false) {
      ToastComponent.showDialog(
          AppLocalizations.of(context)!.product_details_screen_seller_chat_creation_unable_warning,
          gravity: Toast.center,
          duration: Toast.lengthLong);
      return;
    }
                ...
                SliverToBoxAdapter(
                  child: Column(
                      ...
                      children: [
                        ...
                        allow_multivendor.$ ? divider():SizedBox(),
                        allow_multivendor.$ ? InkWell(
                          onTap: () {
                            Navigator.push(context,
                                MaterialPageRoute(builder: (context) {
      ...
      child: Row(
        children: [
          ...
          Padding(
            ...
            child: Text(
              CurrencyFormat.convertToIdr(int.parse(_totalPrice.toString().replaceAll(".00", ""))),
  ...
  Column buildQuantityRow() {
    return Column(crossAxisAlignment:CrossAxisAlignment.start,children: [
      ...
      Padding(
        ...
        child: Text(
          _stock > 0 ? "Tersedia ${_stock} item, jika jumlah > ${_stock} maka sebagian pembelian Pre Order" : "Pembelian Pre Order",
          style: TextStyle(
          ...
          child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return Row(crossAxisAlignment:CrossAxisAlignment.start,children: [
                  Expanded(flex: 1,child: Text("${_wholesaleList[index]['min_qty']}")),
                  Expanded(flex: 1,child: Text("${_wholesaleList[index]['max_qty']}")),
                  Expanded(flex: 1,child: Text(CurrencyFormat.convertToIdr(double.parse(_wholesaleList[index]['price'])))),
                ]);
                    //buildWholesaleItem(index);
              },
            ),
          )
      ]);
  ...
  Row buildMainPriceRow() {
    return Row(
      children: [
        Text(
          CurrencyFormat.convertToIdr(double.parse(_singlePriceString)),
          style: TextStyle(
              color: MyTheme.accent_color,
              fontSize: 16.0,
              fontWeight: FontWeight.w600),
 ...
 Widget buildBottomAppBar(BuildContext context, _addedToCartSnackbar) {
      ...
      children: [
        RatingBar(
          ...
          ratingWidget: RatingWidget(
            full: FaIcon(FontAwesomeIcons.star, color: Colors.amber),
            half: FaIcon(FontAwesomeIcons.starHalf, color: Colors.amber),
            empty:
            FaIcon(FontAwesomeIcons.star, color: Color.fromRGBO(224, 224, 225, 1)),
          ),
  ...
  buildQuantityUpButton() => Container(
        ...
        width: 36,
        child: IconButton(
            icon: FaIcon(FontAwesomeIcons.plus, size: 16, color: MyTheme.dark_grey),
            onPressed: () {
              var jumlah = _stock + _stock_preorder;
              print("cek jumlah: $jumlah");
              if (_quantity < jumlah) {
                _quantity++;
                setState(() {});
                calculateTotalPrice();
  ...
  buildQuantityDownButton() => Container(
      ...
      width: 36,
      child: IconButton(
          icon: FaIcon(FontAwesomeIcons.minus, size: 16, color: MyTheme.dark_grey),
          onPressed: () {
            if (_quantity > 1) {
              _quantity--;
              setState(() {});
