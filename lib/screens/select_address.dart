...
class _SelectAddressState extends State<SelectAddress> {
  ...
  fetchShippingAddressList() async {
    ...
    if (_shippingAddressList.length > 0) {
      ...
      _seleted_origin = _shippingAddressList[0].kec_id;

      _shippingAddressList.forEach((address) {
        if (address.set_default == 1) {
          _seleted_shipping_address = address.id;
          _seleted_origin = address.city_id;
        }
      });
    }
    _faceData = true;
  ...
  onPressProceed(context) async {
    ...
    print("origin: $_seleted_origin");
    Navigator.push(context, MaterialPageRoute(builder: (context) {
      return ShippingInfo(
          owner_id: widget.owner_id!,
          destinasi: _seleted_origin
      );
    })).then((value) {
      onPopped(value);
    });
    ...
    return GestureDetector(
      onTap: () {
        if (_seleted_shipping_address != _shippingAddressList[index].id) {
          ...
          _seleted_origin = _shippingAddressList[index].kec_id;
         // onAddressSwitch();
        }
